...
/**
 * Class which displays rendered previews of roff(7) documents.
 * @internal
 * @class
 */
class TroffView{
	...
	constructor(page = {}){
		...
		this.element.className = "troff-view native-key-bindings";
		...
		this.disposables.add(
			new Disposable(() => window.removeEventListener("resize", onResize)),
			atom.workspace.observeActivePaneItem(onResize),
			atom.workspace.observePanes(pane => pane.onDidChangeFlexScale(onResize))
		);
	...
	static deserialize(attr){
		return new TroffView(attr);
	}
	
	serialize(){
		return {
			deserializer: "TroffView",
			filePath:     this.path,
	...
	/**
	 * Current width of the viewer's element, measured in columns.
	 * @property {Number} width
	 * @readonly
	 */
	get width(){
		return Math.round(this.content.offsetWidth / atom.config.get("editor.fontSize") - 1);
	}
	...
	async render(){
		if(!this.source || !this.isVisible) return;
		
		// Nothing's changed that warrants re-rendering
		const {lastRender, width, path} = this;
		if(lastRender && width === lastRender.width && path === lastRender.path)
			return;
		...
		if(source !== this.content.innerHTML){
			...
			if(!lastRender || lastRender.path !== path)
				this.element.scrollTop = 0;
			this.lastRender = {path, width};
		}
	}
	...
	async openInEditor(){
		if(!this.path) return;
		const container  = atom.workspace.paneContainerForItem(this);
		const activePane = atom.workspace.paneForItem(this);
		const panes      = container.getPanes().filter(pane => pane !== activePane);
		return atom.workspace.open(this.path, panes.length ? {pane: panes.shift()} : {});
	}
