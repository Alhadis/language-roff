...
const {ManPageReference} = require("roff");
...
module.exports = () => [
	...
	atom.workspace.addOpener(uri => {
		const page = ManPageReference.fromURL(uri);
		if(null !== page)
 ...
 * search results as directorie are found.
 */
async function runSearchCmd(){
	...
	return null;
	  /*************************************************************************
	  	TODO: #1, we break the chunk of text `man -k' gives that looks like:
	...
	TODO #2: Make sure that parsing is somewhat reliable beteen platforms.
			 Use the extracted `topic(section)' as a source of truth if all else fails.'
	TODO #3: Set it so a user picking a selection has the page asynchronously
